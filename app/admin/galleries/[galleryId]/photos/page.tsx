      ...
      <div className="rounded-2xl border bg-card p-8 shadow-sm space-y-6">
        ...
        {files.length > 0 && (
          <div className="space-y-4">
            <h3 className="font-medium text-sm text-muted-foreground">Selected Photos ({files.length})</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {files.map((item, index) => (
                <div key={index} className="group relative overflow-hidden rounded-lg border bg-muted/30">
                  <div className="relative h-40 w-full">
                    <Image src={item.preview} alt="Preview" fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 25vw" className="object-cover transition-transform group-hover:scale-[1.03]" />
                  </div>
                  <button onClick={() => removeFile(index)} className="absolute top-2 right-2 rounded-full bg-black/60 text-white text-xs px-2 py-1 opacity-0 group-hover:opacity-100 transition">
                    ✕
                  </button>
                </div>
              ))}
        ...
        {fetchingPhotos ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {existingPhotos.map((photo) => (
              <div key={photo.id} className="relative group overflow-hidden rounded-lg border bg-muted/10">
                <div className="relative h-40 w-full">
                  <Image src={photo.image_url} alt={photo.name} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 25vw" className="object-cover transition-transform group-hover:scale-[1.03]" />
                </div>
                <button onClick={() => deletePhoto(photo.id, photo.public_id)} className="absolute top-2 right-2 rounded-full bg-red-600 text-white text-xs px-2 py-1 opacity-0 group-hover:opacity-100 transition">
